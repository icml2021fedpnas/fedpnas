...
from SNAS.model_edge_all import NetworkCIFAR as Network
...
SNAS.utils.create_exp_dir(generate_date,args.save, scripts_to_save=glob.glob('*.py'))
...
def main():
    ...
    logging.info("param size = %fMB", SNAS.utils.count_parameters_in_MB(model))
    ...
    train_transform, valid_transform = SNAS.utils._data_transforms_cifar10(args)
    ...
    for epoch in range(args.epochs):
        ...
        SNAS.utils.save(model, os.path.join(args.save, 'weights.pt'))
...
def train(train_queue, model, criterion, optimizer, epoch):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        prec1, prec5 = SNAS.utils.accuracy(logits, target, topk=(1, 5))
        n = input.size(0)
