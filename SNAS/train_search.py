...
if args.distributed:
    ...
    from SNAS.dist_util_torch import init_dist, broadcast_params, reduce_gradients, reduce_tensorgradients, part_reduce_gradients
...
class neural_architecture_search():
    def __init__(self, args):
        ...
        if self.rank == 0:
            SNAS.utils.create_exp_dir(generate_date, self.path, scripts_to_save=glob.glob('*.py'))
        ...
        if self.rank == 0:
            logging.info("param size = %fMB", SNAS.utils.count_parameters_in_MB(self.model))
    ...
    def init_loaddata(self):
        ...
        train_transform, valid_transform = SNAS.utils._data_transforms_cifar10(self.args)
    ...
    def main(self):
        ...
        self.temp_scheduler = SNAS.utils.Temp_Scheduler(self.args.epochs, self.model._temp, self.args.temp, temp_min=self.args.temp_min)
        ...
        for epoch in range(self.args.epochs):
            ...
            if self.rank == 0:
                ...
                SNAS.utils.txt('genotype', self.args.save, txt_name, str(genotype_edge_all), generate_date)
            ...
            if self.rank == 0:
                ...
                SNAS.utils.save(self.model, os.path.join(self.path, 'weights.pt'))
    ...
    def train(self, epoch, logging):
        objs = SNAS.utils.AvgrageMeter()
        top1 = SNAS.utils.AvgrageMeter()
        top5 = SNAS.utils.AvgrageMeter()
        grad = SNAS.utils.AvgrageMeter()
